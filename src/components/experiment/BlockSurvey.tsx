...
import { BLOCK_SURVEY_QUESTIONS } from '../../core/experiment/config';

interface Props {
  blockId: 'block1' | 'block2';
}

export const BlockSurvey: React.FC<Props> = ({ blockId }) => {
  const [answers, setAnswers] = useState<Record<string, string | number>>({});
  const submitSurvey = useExperimentStore((state) => state.submitSurvey);
  const activeMode = useExperimentStore((state) => (blockId === 'block1' ? state.session?.block1Mode : state.session?.block2Mode));
  ...
  const allRequiredAnswered = BLOCK_SURVEY_QUESTIONS.every(
    ...
    <div style={{
      ...
      <div style={{ marginBottom: '24px', borderBottom: '1px solid #313244', paddingBottom: '16px' }}>
        <h2 style={{ fontSize: '22px', fontWeight: 700, color: '#f5c2e7', margin: '0 0 6px 0' }}>
          Опрос по блоку {blockId === 'block1' ? '1' : '2'} ({modeTitle})
        </h2>
        <p style={{ color: '#a6adc8', fontSize: '14px', margin: 0 }}>
          Оцените ваши впечатления от взаимодействия с AI-тьютором в завершенном блоке.
        </p>
      </div>

      <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
        {BLOCK_SURVEY_QUESTIONS.map((q) => (
        ...
        <button
          ...
          style={{
            ...
            borderRadius: '10px',
            border: 'none',
            backgroundColor: allRequiredAnswered ? '#cba6f7' : '#45475a',
